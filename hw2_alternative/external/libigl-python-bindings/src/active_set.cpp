...
npe_begin_code()
  ...
  Eigen::VectorXd Z_copy;
  if (Z.size() > 0)
    Z_copy = Z.template cast<double>();

  auto res = igl::active_set(A_copy, B_copy, known_copy, Y_copy, Aeq_copy, Beq_copy, Aieq_copy, Bieq_copy, lx_copy, ux_copy, params, Z_copy);
  EigenDenseLike<npe_Matrix_B> Z_row_major = Z_copy.template cast<typename npe_Matrix_B::Scalar>();
